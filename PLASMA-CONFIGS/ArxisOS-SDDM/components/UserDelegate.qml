...
Item {
    ...
    Item {
        ...
        // Fallback: try to load iconSource as an image file (for file:// paths)
        Image {
            id: faceIconImage
            source: iconSource
            sourceSize: Qt.size(faceSize, faceSize)
            fillMode: Image.PreserveAspectCrop
            visible: (face.status == Image.Error || face.status == Image.Null) && (status == Image.Ready)
            anchors.fill: parent
            anchors.margins: Kirigami.Units.gridUnit * 0.5
        }

        // Final fallback: use Kirigami icon for icon names (like "user-identity")
        Kirigami.Icon {
            id: faceIcon
            source: "user-identity"
            visible: (face.status == Image.Error || face.status == Image.Null) && (faceIconImage.status != Image.Ready)
            anchors.fill: parent
            anchors.margins: Kirigami.Units.gridUnit * 0.5
        }
    }
